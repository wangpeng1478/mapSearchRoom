<template>
  <div class="roomlist-wrap" :class="roomListTransition ? 'roomlist-transition' : ''"
    ...
    <div class="village">
        ...
        <a :href="currentCity.url+'/'+currentCity.cityPinyin+'/xiaoqu/gk_v'+roomList[0].villageId" target="_blank" class="village-info">{{roomList[0].villageName}}（{{roomList.length}}间）</a>
        </template>
    </div>
    ...
    <div class="roomlist" :class="{'height80' : translateState==1}" v-if="roomList.length!=0">
      ...
      <a :href="currentCity.url+'/'+currentCity.cityPinyin+'/easy/'+room.roomId" target="_blank" v-for="room in roomList" :key="room.roomId">
