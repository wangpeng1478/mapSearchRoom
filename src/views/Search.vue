<template>
  <div>
    <div class="input-row">
      <input id="suggestId" type="text" placeholder="请输入小区/区域/地图" maxlength="50" v-model="searchValue">
      <!-- @input="handleInput" -->
    ...
    <div class="search-tag" v-show="searchValue==''">
      <div class="search-tag-list" v-if="isRegion">
        ...
        <ul class="tag-list" v-if="searchTagHistory.length!=0">
          <li @click="handleSearchTag(index,'searchTagHistory')" v-for="(historyTag,index) in searchTagHistory"
            :key="index">{{historyTag.keyWords}}</li>
        </ul>
      ...
      <!-- //个性找房历史 -->
       <div class="search-tag-list"  v-if="!isRegion">
        ...
        <ul class="tag-list" v-if="pointTagHistory.length!=0">
          <li v-for="(pointTag,index) in pointTagHistory"
            :key="index">{{pointTag.name}}</li>
        </ul>
        <div v-else class="nolist">
          <img src="../assets/images/none.png">
          <p>暂无搜索历史</p>
      ...
      <div class="search-tag-list" v-if="isRegion">
        ...
        <ul class="tag-list" v-if="hotSearch.length!=0">
          <li @click="handleSearchTag(index,'hotSearch')" v-for="(hotTag,index) in hotSearch" :key="index">
            {{hotTag.keyWords}}</li>
        </ul>
        <div v-else class="nolist">
          <img src="../assets/images/none.png">
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.isRegion) {
        ...
        //个性找房
        let pointStorageName = "pointTagHistory" + this.currentCity.cityId;
        if (localStorage[pointStorageName]) {
          this.pointTagHistory = JSON.parse(localStorage[pointStorageName]);
        } else {
    ...
    methods: {
      ...
      handleAcResult(idx) {
        ...
        let _this = this;
        let myValue = this.acResult[idx].city + this.acResult[idx].district + this.acResult[idx].business
        this.myGeo.getPoint(myValue, function (res) {
          if (res) {
            let point = res;
      ...
      savePointStorage(point) {
        ...
        localStorage.setItem(pointStorageName, pointTagHistory);
      },
      clearHistory() {
        localStorage.removeItem("searchTagHistory" + this.currentCity.cityId);
        this.searchTagHistory = [];
      },
      handleInput(e) {
        let params = new URLSearchParams({
